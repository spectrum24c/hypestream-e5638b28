      ...
      <main className="pb-8 pt-20">
        <div className="container mx-auto px-4">
          ...
          {loading ? (
            ...
            <div className="text-center py-12 bg-card/20 rounded-lg border border-border">
              <h2 className="text-2xl font-semibold mb-4">No favorites yet</h2>
              <p className="text-muted-foreground mb-6">Movies and shows you favorite will appear here</p>
              <Button 
                onClick={() => navigate('/')} 
              ...
              {favorites.map((favorite) => (
                <div key={favorite.id} className="relative group">
                  <MovieCard 
                    ...
                    voteAverage={favorite.vote_average || undefined}
                    isTVShow={favorite.is_tv_show}
                    overview={favorite.overview}
                    onClick={() => handleMovieClick(favorite)}
                  />
                  <button 
                    className="absolute top-2 right-2 bg-destructive text-destructive-foreground rounded-full w-8 h-8 flex items-center justify-center cursor-pointer transition-opacity sm:opacity-0 sm:group-hover:opacity-100 opacity-100 z-10"
                    onClick={(e) => {
                      e.stopPropagation();
